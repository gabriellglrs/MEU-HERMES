...
---
# --- agentskill.sh ---
# slug: nousresearch/spike
# owner: NousResearch
# source: https://agentskill.sh/nousresearch/spike
# ---
...
Spikes are not research-free — you research enough to pick the right approach, then you build.
...
Use Hermes tools:
- `web_search(...)` — find candidates
- `web_extract(urls=[...])` — read the actual docs
- `terminal("pip show ...")` — check what's installed
...
**Bias toward something the user can interact with.** Default choices, in order of preference:
...
When two approaches answer the same question, build them **back to back**, then do a head-to-head comparison.
