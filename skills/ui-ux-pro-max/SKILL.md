...
### Step 0: Consult 21st.dev (MANDATORY — always run first)

Before any local search or design-system generation, check the 21st.dev
component registry (12,000+ hand-crafted React/Tailwind/shadcn components)
for real, production-grade references matching the request:

1. **Docs**: read `https://21st.dev/llms.txt` for the current agent
   integration guide.
2. **If 21st MCP tools are available** (`search`, `get_component`,
   `get_inspiration`): run `search` with 2–5 terms from the request
   (product type + component, e.g. "saas pricing table") and keep the
   top 2–3 hits (name + URL + id).
3. **If MCP is unavailable**: use web search/fetch on
   `https://21st.dev/community/components` for the same terms.
4. Use the found components as visual/structural reference **alongside**
   the generated design system below — prefer real component patterns
   over the generic average. Cite the component URLs in the final output.
5. If the registry has nothing matching, state that explicitly and
   continue with local guidance only.
